...
import { doesTrigger } from 'vs/workbench/contrib/debug/browser/deSyntConstants';
import { DesyntHistoryView } from 'vs/workbench/contrib/debug/browser/desyntHistoryView';
...
//desynt fix workbench.debug.action.focusWatchView
viewsRegistry.registerViews([{ id: DESYNT_VIEW_ID, name: nls.localize('desynt', "DeSynt"), containerIcon: icons.watchViewIcon, ctorDescriptor: new SyncDescriptor(DeSyntView), order: 20, weight: 10, canToggleVisibility: true, canMoveView: true, focusCommand: { id: 'workbench.debug.action.focusWatchView' }, when: ContextKeyExpr.and(CONTEXT_DEBUG_UX.isEqualTo('default'), doesTrigger ? ContextKeyExpr.true() : ContextKeyExpr.false()) }], viewContainer);
...
viewsRegistry.registerViews([{ id: DESYNT_HISTORY_VIEW_ID, name: nls.localize('desyntHistory', "Desynt History"), containerIcon: icons.loadedScriptsViewIcon, ctorDescriptor: new SyncDescriptor(DesyntHistoryView), order: 35, weight: 5, canToggleVisibility: true, canMoveView: true, collapsed: true, when: CONTEXT_DEBUG_UX.isEqualTo('default') }], viewContainer);
